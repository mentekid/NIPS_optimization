...
\newcommand*\rot{\multicolumn{1}{R{60}{2em}}}
...
class ObjectiveFunction {
 ...
 void Shuffel() {
  order = arma::shuffle(
   arma::linspace<arma::uvec>(0, 3, 4));
 }

 size_t NumFunctions() { return 4; }

 double Evaluate(const arma::mat& p,
  size_t s, size_t b) {
  double cost = 0;
  for (size_t i = s; i < s + b; i++) {
   cost += std::pow(p(i), 2) +
    bi(order(i)) * p(order(i)) +
    in(order(i));
  }
  return cost;
 }

 void Gradient(const arma::mat& p,
  size_t s, size_t b, arma::sp_mat& g) {
  g.zeros(arma::size(p));

  for (size_t i = s; i < s + b; i++) {
   g(i) += 2 * p(i) + bi(order(i));
 }
}
\end{minted}
\vspace{2\baselineskip}
% \captionof{listing}{Distinct parabola sparse function.}
\end{minipage}
\begin{minipage}[t]{0.49\textwidth}
\centering
\begin{minted}[fontsize=\footnotesize]{c++}
...
\end{minted}
% \captionof{listing}{Optimizer class.}
\end{minipage}
